...
def run_regime_detection(df):
    ...
    df['market_regime'] = df['market_regime'].ffill().astype(int)
...
def train_challenger_models(df_train):
    ...
    for regime in regimes:
        ...
        model = lgb.LGBMClassifier(objective='multiclass', num_class=3, class_weight='balanced', seed=42, verbose=-1)
...
def backtest_challenger(df_with_signals):
    ...
    backtester = VectorizedBacktester(price_data, signals, initial_capital=100000, strategy_overlay=overlay)
