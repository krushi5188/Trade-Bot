...
**Status:** COMPLETE.
**Key Outcomes:**
- A full regime detection pipeline was implemented in `src/tier6/regime_detection_pipeline.py`.
- The pipeline successfully uses a Gaussian Mixture Model to identify distinct market regimes based on volatility and momentum.
- It then trains a "specialist" model for each regime and dynamically selects the correct one for prediction.
- Backtesting the final strategy (including transaction costs) resulted in a slightly negative return. This is a critical finding, as it proves that the current feature set is insufficient to generate alpha, even with this advanced, adaptive architecture.
**Architecture:**
...
## Tier 7: Hypothesis-Driven Feature Engineering
**Objective:** To move beyond generic indicators and discover alpha by scientifically testing specific, observable market behaviors.
**Status:** IN-PROGRESS.
**Key Outcomes:**
- A reusable hypothesis-testing framework was built in `src/tier7/hypothesis_tester.py`.
- **Hypothesis 1 (London Lunch Pullback):** This hypothesis was tested on our existing hourly data for both BTC and Gold. The phenomenon was found to be statistically insignificant (<0.3% occurrence), proving it is not a source of alpha at this timeframe.
- **Critical Finding (Data Limitation):** It was determined that testing short-term hypotheses is not possible with the project's current hourly data. All attempts to source free, high-quality, high-frequency data (1-min, 5-min) have failed due to API limitations or environment incompatibility (`MetaTrader5`).
- **Next Step:** The project is currently blocked on acquiring high-frequency data. The immediate next step is for the user to provide tick data exported from MT5, which will unblock this entire tier of research.
