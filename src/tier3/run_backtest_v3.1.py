# This script runs a backtest on the V3.1 (tuned with CV) model and V2 features.
...
def run_v3_1_backtest():
    """
    Loads the V3.1 tuned model and V2 features, and runs a backtest.
    """
    # 1. Define Paths
    PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
    FEATURE_PATH = os.path.join(PROJECT_ROOT, 'data/processed/features_v2_final.parquet')
    MODEL_PATH = os.path.join(PROJECT_ROOT, 'src/tier3/modeling/lgbm_v3.1_tuned_cv_model.txt')
    OUTPUT_DIR = os.path.join(PROJECT_ROOT, 'analysis/backtest_reports')
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    # 2. Load Model and Data
    model_name = "LightGBM V3.1 (Tuned with CV)"
    ...
    # Use the final 20% of data as the hold-out test set
    ...
    plot_filename = "lgbm_v3.1_tuned_cv_equity_curve.png"
